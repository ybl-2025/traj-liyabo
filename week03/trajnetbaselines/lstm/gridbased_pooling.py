...
class GridBasedPooling(torch.nn.Module):
    def __init__(self, cell_side=2.0, n=4, hidden_dim=128, out_dim=None,
                 ...
                 same_cell_aggregation='overwrite'):
        """
        ...
        self.norm = norm
        self.pool_scale = 1.0
        if same_cell_aggregation not in ('overwrite', 'mean'):
            raise ValueError("same_cell_aggregation must be 'overwrite' or 'mean'")
        self.same_cell_aggregation = same_cell_aggregation
    ...
    def occupancy(self, obs, other_values=None, past_obs=None):
        ...
        # Keep invalid neighbours from indexing outside the flattened grid.
        # Do not mutate ``other_values`` because the mean branch must preserve
        # its autograd connection to every valid neighbour.
        oij = torch.where(range_mask.unsqueeze(-1), oij, torch.zeros_like(oij))
        ...
        num_grids = batch_size * num_tracks
        num_cells = self.n**2 * self.pool_size**2
        aggregation = getattr(self, 'same_cell_aggregation', 'overwrite')

        if aggregation == 'mean':
            # Accumulate all neighbours that land in the same cell, then
            # divide by the number of valid contributors.  scatter_add keeps
            # the operation differentiable for every neighbour feature.
            valid = range_mask.view(num_grids, num_tracks-1)
            indices = oi.unsqueeze(-1).expand(-1, -1, self.pooling_dim)
            contributions = other_values * valid.unsqueeze(-1).to(other_values.dtype)
            occ_sum = torch.zeros(num_grids, num_cells, self.pooling_dim,
                                  dtype=other_values.dtype, device=obs.device)
            occ_sum.scatter_add_(1, indices, contributions)
            counts = torch.zeros(num_grids, num_cells, 1,
                                 dtype=other_values.dtype, device=obs.device)
            counts.scatter_add_(1, oi.unsqueeze(-1),
                                valid.unsqueeze(-1).to(other_values.dtype))
            background = self.constant * torch.ones_like(occ_sum)
            occ = torch.where(counts > 0, occ_sum / counts.clamp_min(1), background)
        else:
            # Original upstream behaviour: repeated indices overwrite rather
            # than explicitly aggregating same-cell neighbours.
            masked_values = torch.where(
                range_mask.view(num_grids, num_tracks-1, 1),
                other_values,
                self.constant * torch.ones_like(other_values),
            )
            occ = self.constant*torch.ones(num_grids, num_cells, self.pooling_dim, device=obs.device)
            occ[torch.arange(occ.size(0)).unsqueeze(1), oi] = masked_values
